...
| `JEV_SUBAGENT_MIN_TIER` | Claude | Lowest tier a routed subagent may land on: `haiku`/`fast`, `sonnet`/`balanced`, `opus`/`strong`. Unset by default. Manual prompt overrides still win; the main agent is unaffected. Independently of this setting, a new subagent whose Jev answer is low-confidence starts on `sonnet` (effort `high` when Jev asked for a stronger model) rather than on the strong model it arrived with. |
| `JEV_SUBAGENT_MAX_TIER` | Claude | Highest route a routed subagent may land on: a tier (`sonnet`, same values as the minimum) or a tier with an effort (`opus-low`, `sonnet-medium`). A stronger pick is lowered to that tier at `high` effort, or to exactly the given effort; a default-effort pick at the capped tier counts as stronger. An effort the model does not support leaves the route unchanged. Unset by default; manual prompt overrides still win and the main agent is unaffected. |
| `JEV_MAIN_1M` | Claude | Set `1` to give the main agent a 1M-token context window. The router row is registered as `jev-router[1m]`, so Claude Code sizes the window and compacts at 1M; main-agent turns on Sonnet or Opus get the `context-1m-2025-08-07` beta, and subagents and Haiku turns have it removed. A main agent held on Haiku past about 180k tokens is moved to Sonnet, because Haiku has no 1M mode. |
| `JEV_MAIN_PROFILES` | Claude | Comma list of profiles the main agent may be routed to, e.g. `opus-*`. `*` wildcards work and a bare tier means all its efforts. If nothing matches, all profiles stay allowed. A main-agent turn that requests `xhigh` or `max` effort (e.g. ultracode) is never routed below that effort. Unset means all profiles, including `sonnet-xhigh` and `opus-low`. |
| `JEV_SUBAGENT_PROFILES` | Claude | The same for subagents, e.g. `sonnet-*,opus-low`. A subagent held on a profile outside the list moves to the nearest allowed one. |
| `JEV_FOLLOWUP_HOLD_CHARS` | Claude | A main-agent message of at most this many characters in an ongoing conversation (e.g. "ok do that", "continue") never downgrades the current model; upgrades still apply. Default `200`; `0` disables. |
