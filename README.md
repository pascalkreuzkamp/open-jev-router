...
| `JEV_SUBAGENT_MAX_TIER` | Claude | Highest tier a routed subagent may land on, same values as the minimum. A stronger pick is lowered to that tier at `high` effort. Unset by default; manual prompt overrides still win and the main agent is unaffected. |
