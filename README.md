...
| `JEV_SUBAGENT_MIN_TIER` | Claude | Lowest tier a routed subagent may land on: `haiku`/`fast`, `sonnet`/`balanced`, `opus`/`strong`. Unset by default. Manual prompt overrides still win; the main agent is unaffected. Independently of this setting, a new subagent whose Jev answer is low-confidence starts on `sonnet` rather than staying on the strong model it arrived with. |
